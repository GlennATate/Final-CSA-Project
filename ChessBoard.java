...
public class ChessBoard extends JLayeredPane 
{
	...
	private class MouseControl extends MouseAdapter 
  {
		...
		public void mouseReleased(MouseEvent e) 
    {
			if (targetPiece == null) 
      {
			...
			// remove(targetPiece);
			JPanel finalMovePanel = (JPanel) chessBoard.getComponentAt(e.getPoint());
			findDestination(finalMovePanel);
			if (targetPanel == finalMovePanel) 
      {
				targetPanel.add(targetPiece);
				targetPanel.revalidate();
				System.out.println("*** DID NOT MOVE - SAME TURN ***");
				remove(targetPiece);
				repaint();
				return;
			} 
					...
					for (int roww = 0; roww < 8; roww++) 
          {
						for (int coll = 0; coll < 8; coll++) 
            {
							if (chessBoardGrid[roww][coll] == finalMovePanel) 
              {
								row = roww;
								col = coll;
								if (currentPiece.isValid(gameBoard, startRow, startCol, roww, coll)) 
                {
									if ("King".equals(currentPiece.getType())) 
                  {
										if (inCheck) 
                    {
											if (isKingChecked(getDummyBoard(startRow, startCol, roww, coll))) 
                      {
												targetPanel.add(targetPiece);
												targetPanel.revalidate();
												remove(targetPiece);
									...
									if (finalMovePanel.getComponentCount() != 0) 
                  {
									...
									gameBoard = chessGame.move(null, startRow, startCol, roww, coll);
	...
	public boolean checkCheck(Square[][] board, int startRow, int startCol, int kingX, int kingY) 
  {
		...
		for (int i = 0; i < 8; i++) 
    {
			for (int j = 0; j < 8; j++) 
      {
        ...
        else if (current.isWhite == board[i][j].getPiece().getPlayer().isWhite) 
        {
        ...
        else if (current.isWhite == board[i][j].getPiece().getPlayer().isBlack) 
        {
					if (numMoves >= 4) 
          {
						if (chessGame.checkCheckMate(board[i][j].getPiece().getPlayer(), gameBoard)) 
            {
							remove(chessBoard);
							adapter = null;
							if (playerTurn) 
	...
	public boolean isKingChecked(Square[][] board) 
  {
		...
		for (int i = 0; i < 8; i++) 
    {
			for (int j = 0; j < 8; j++) 
      {
        ...
        else if (currentPlayer.isWhite == board[i][j].getPiece().getPlayer().isBlack) 
        {
					Piece piece = board[i][j].getPiece();
					if (piece.isValid(board, i, j, currentKingX, currentKingY)) 
          {
						System.out.println(piece.getType() + " KING IN CHECK!");
						return true;
					}
				}
			}
	...
	private void findKing(Square[][] board) 
  {
	...
	public Square[][] getDummyBoard(int startRow, int startCol, int finalRow, int finalCol) 
  {
